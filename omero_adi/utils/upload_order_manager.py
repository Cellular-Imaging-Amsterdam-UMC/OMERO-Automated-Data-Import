...
class UploadOrderManager:
    ...
    def validate_order_attributes(self):
        ...
        """
        required_attributes = ['Group', 'Username', 'UUID', 'DestinationID', 'DestinationType']
        ...
        try:
            self.order_info['DestinationID'] = int(self.order_info['DestinationID'])
        except (ValueError, TypeError):
            error_message = f"'DestinationID' must be a valid integer, got: {self.order_info['DestinationID']}"
            self.logger.error(error_message)
            raise ValueError(error_message)

        self.logger.info("All required attributes are present in the upload order, 'DestinationType' is valid, and 'DestinationID' is an integer.")
    ...
    def _create_file_names_list(self):
        ...
        if 'Files' in self.order_info:
            ...
            self.logger.debug(f"Created file_names list with {len(self.order_info['FileNames'])} entries")
        else:
